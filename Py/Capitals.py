Country_Capitals={
    'tirane': 'albania', 'algiers': 'algeria', 'andorra la vella': 'andorra', 'luanda': 'angola',
...
start=1
while start ==1:
    capital=input("Please enter anay capital in the world!").lower()
    if capital in Country_Capitals:
     start =0
    else:
     print(f"{capital} is not a capital")
score =1
game =1
while game ==1:
    print(f"Correct, {capital.title()} is the capital of {Country_Capitals[capital].title()}")
    last_letter=capital[-1]
    capital=input(f" What is the capital in the wolrd starts with '{last_letter.upper()}' letter?").lower()
    if capital[0]==last_letter and capital in Country_Capitals:
        score+=1
    else:
       game=0
       for correct_capital in Country_Capitals:
          if correct_capital[0] == last_letter:
            print(f"Wrong, '{capital}' is not corect, you can input {correct_capital.title()} the capital of {Country_Capitals[correct_capital].title()}")
       break
